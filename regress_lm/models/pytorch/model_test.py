...
class ModelTest(absltest.TestCase):
  ...
  def test_log_prob_and_update(self):
    batch = self.model.convert_examples([
        core.Example(x="hello", y=1.0),
        core.Example(x="hello", y=1.0),
    ])
    log_probs_before = self.model.log_prob(batch)
    self.assertEqual(log_probs_before.shape, (2,))
    self.assertAlmostEqual(log_probs_before[0].squeeze().item(), -18.54, 1)

    # Update the model. Logprob should improve.
    self.model.update(batch)
    log_probs_after = self.model.log_prob(batch)
    self.assertAlmostEqual(log_probs_after[0].squeeze().item(), -13.18, 1)
  ...
  def test_decode(self):
    ...
    self.assertAlmostEqual(output_floats[0, 0], -0.2843)
    self.assertAlmostEqual(output_floats[0, 1], 23270000.0)

    self.assertAlmostEqual(np.median(output_floats), 0.8632)

    # After updating, the median should get closer to target y.
    for _ in range(20):
      self.model.update(batch)
    _, output_floats = self.model.decode(batch, num_samples=128)
    self.assertAlmostEqual(np.median(output_floats), 2.313)
