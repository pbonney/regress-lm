...
def _get_version():
  with open('regress_lm/__init__.py') as fp:
    for line in fp:
      if line.startswith('__version__'):
        g = {}
        exec(line, g)  # pylint: disable=exec-used
        return g['__version__']
    raise ValueError('`__version__` not defined in `regress_lm/__init__.py`')
...
setuptools.setup(
    ...
    description='RegressLM',
    author='RegressLM Team',
    author_email='regress_lm@google.com',
    install_requires=_parse_requirements('requirements.txt'),
    packages=setuptools.find_namespace_packages(
        include=['regress_lm*'], exclude=['*_test.py']
    ),
